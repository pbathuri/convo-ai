...
const BANNED_SOURCE_TYPES = new Set([
  "glassdoor_full_review",
  "linkedin_scrape",
]);

const RAG_ALLOWED: AllowedUsage[] = ["ui_visible", "rag_only_no_display"];

export function validateKbImport(g: KbImportGovernance): {
  ok: boolean;
  reason?: string;
} {
  ...
  if (
    g.licenseStatus === "unknown" &&
    (!g.allowedUsage || !RAG_ALLOWED.includes(g.allowedUsage))
  ) {
    return {
      ok: false,
      reason:
