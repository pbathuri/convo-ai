...
export async function scoreTranscript(opts: {
  ...
  transcript: string;
  kbContext?: string[];
}): Promise<{ output: ScoreOutput; modelName: string; latencyMs: number }> {
  ...
  if (!apiKey) {
    const stub: ScoreOutput = {
      overallScore: 72,
      dimensions: rubric.dimensions.map((d) => ({
        name: d.name,
        score: 70,
        rationale: "Stub score — set GOOGLE_AI_STUDIO_KEY for live scoring.",
      })),
      ...
      evidence: [{ quote: opts.transcript.slice(0, 120) || "(no transcript)", dimension: "Sample" }],
      confidence: 0.5,
      nextDrill: persona?.openingQuestion,
    };
    return { output: stub, modelName: "stub", latencyMs: Date.now() - start };
  }
